...
class Orderable_Location_Single {
	...
	public function get_service_hours( $service_type = null, $is_admin = false, $skip_zone = false ) {
		...
		if ( 'pickup' !== $original_service_type && false !== $zone_id && ! $skip_zone ) {
	...
	public function get_open_days() {
		...
		if ( ! is_null( $open_days_cache ) ) {
			return $open_days_cache;
		}
		 ...
		 */
		$open_days_cache = apply_filters( 'orderable_location_get_open_days', $open_days, $this );
	...
	public function get_open_hours() {
		...
		if ( ! is_null( $open_hours_cache ) ) {
			return $open_hours_cache;
		}
		 ...
		 * @param Orderable_Location_Single $location   Location object.
		 */
		$open_hours_cache = apply_filters( 'orderable_location_get_open_hours', $open_hours, $this );
	...
	public function get_service_dates( $type = false, $ignore_needs_shipping = false ) {
		 ...
		 * @param int|false  $this           The location instance.
		 *
