	...
	public static function modify_package_rates( $rates, $package ) {
		...
		foreach ( $rates as $rate_id => $rate ) {
			...
			if ( false === strpos( $rate_id, 'pickup' ) ) {
				...
				if ( ! $selected_location->is_service_enabled( 'delivery' ) ) {
					unset( $rates[ $rate_id ] );
				}
			} else {
				$pickup_rates[ $rate_id ] = $rate;

				if ( ! $selected_location->is_service_enabled( 'pickup' ) ) {
					unset( $rates[ $rate_id ] );
				}
			}
		}
		...
		if ( $selected_location->is_service_enabled( 'pickup' ) && empty( $pickup_rates ) ) {
