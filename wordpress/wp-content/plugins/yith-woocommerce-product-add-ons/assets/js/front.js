...
jQuery( document ).ready(
	function ($) {
		...
		var wapoDOM = {
			...
			variationForm		: '.yith-wapo-popup form.variations_form',
		...
		addToCart			= function(e) {
			...
			var productId     = $( wapoDOM.variationForm ).find( '.product_id' ).val();
			var variationId   = $( wapoDOM.variationForm ).find( '.variation_id' ).val();
					...
					success : function( response ) {
						if ( response ) {
							...
							if ( typeof wc_add_to_cart_variation_params !== 'undefined' ) {
								$(wapoDOM.variationForm).each(function () {
		...
		replaceImageAction 			= function( optionWrapper, reset = false ) {
			...
			if ( typeof optionWrapper.data( 'replace-image' ) !== 'undefined' && optionWrapper.data( 'replace-image' ) != '' ) {
				...
				$( defaultPath ).attr( 'src', replaceImageURL );
				$( defaultPath ).attr( 'srcset', replaceImageURL );
				$( defaultPath ).attr( 'data-src', replaceImageURL );
				$( zoomMagnifier ).css( 'background-image', 'url(' + replaceImageURL + ')' );
				$( defaultPath ).attr( 'data-large_image', replaceImageURL );
			...
			if ( reset && typeof( $( defaultPath ).attr( 'wapo-original-img' ) ) != 'undefined' ) {
				...
				$( defaultPath ).attr( 'src', originalImage );
				$( defaultPath ).attr( 'srcset', originalImage );
				$( defaultPath ).attr( 'data-src', originalImage );
				$( defaultPath ).attr( 'data-large_image', originalImage );
				$( zoomMagnifier ).css( 'background-image', 'url(' + originalZoom + ')' );

				// Reset gallery position when add-on image change
				if ( $( wapoDOM.wcProductGallery ).find( '.woocommerce-product-gallery__image' ).length > 0 ) {
					$( wapoDOM.wcProductGallery ).trigger( 'woocommerce_gallery_reset_slide_position' );
				}
				$( wapoDOM.wcProductGallery ).trigger( 'woocommerce_gallery_init_zoom' );
				$( document ).trigger( 'yith-wapo-after-replace-image' );
